...
module.exports = async function handler(req, res) {
  ...
  if (!process.env.BLOB_READ_WRITE_TOKEN) {
    res.status(200).json({ productos: fallback, actualizado: null, fuente: 'inicial' })
    return
  }
  ...
  try {
    ...
    res.status(200).json({ productos, actualizado: resultado.blob.uploadedAt, fuente: 'admin' })
  } catch (err) {
    res.status(200).json({
      productos: fallback,
      actualizado: null,
